...
#
# The tree on disk is a build tree: about half of it is object files, static
# libs and wine's per-dll test suites, none of which are needed to run a game.
# Dropping those takes ~5.7 GB down to ~2.3 GB before compression.
#
# nls/ and fonts/ are symlinks pointing back into the wine source tree. Ship
# them as links and they dangle on someone else's machine, which shows up as
# a wineserver l_intl.nls failure and fontless, tiny game windows - so they get
# copied for real.
set -euo pipefail
